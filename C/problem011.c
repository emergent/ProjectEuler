...
void problem011() {
    ...
    ans = 0;
    tmp = 0;
    for (i = 0; i < SIDE_LENGTH; i++) { /* 横向きの走査 */
        n = numbers[i];
        for (j = 0; j < SIDE_LENGTH - 3; j++) {
            tmp = *(n + j) * *(n + j + 1) * *(n + j + 2) * *(n + j + 3);
            if (tmp > ans) {
                ans = tmp;
            }
        }
    }
    for (i = 0; i < SIDE_LENGTH - 3; i++) { /* 縦向きの走査 */
        n = numbers[i];
        for (j = 0; j < SIDE_LENGTH; j++) {
            tmp = *(n + j) * *(n + j + SIDE_LENGTH * 1) *
                  *(n + j + SIDE_LENGTH * 2) * *(n + j + SIDE_LENGTH * 3);
            if (tmp > ans) {
                ans = tmp;
            }
        }
    }
    for (i = 0; i < SIDE_LENGTH - 3; i++) { /* 右下向きの斜めの走査 */
        n = numbers[i];
        for (j = 0; j < SIDE_LENGTH - 3; j++) {
            tmp = *(n + j) * *(n + j + 1 + SIDE_LENGTH * 1) *
                  *(n + j + 2 + SIDE_LENGTH * 2) *
                  *(n + j + 3 + SIDE_LENGTH * 3);
            if (tmp > ans) {
                ans = tmp;
    ...
    for (i = 3; i < SIDE_LENGTH; i++) { /* 左下向きの斜めの走査 */
        n = numbers[i];
        for (j = 0; j < SIDE_LENGTH - 3; j++) {
            tmp = *(n + j) * *(n + j - 1 + SIDE_LENGTH * 1) *
                  *(n + j - 2 + SIDE_LENGTH * 2) *
